   ...
   @Bean
    public App initSlackApp() throws Exception {
               ...
               ctx.respond(asBlocks(
                       ...
                       actions(actions -> actions
                               .elements(asElements(
                                       ...
                                       button(b -> b.actionId("Endpoint URL REST").text(plainText(pt -> pt.text("Endpoint URL"))).value("https://endpointurl.com")),
                                       button(b -> b.actionId("Jenkins Job SOAP REST").text(plainText(pt -> pt.text("jenkins"))).value("http://jenkinsjob.com")),
       ...
       app.blockAction("Endpoint URL SOAP", (req, ctx) -> {
           String value = req.getPayload().getActions().get(0).getValue();
           if (req.getPayload().getResponseUrl() != null) {
               ctx.respond("Please find the endpoint URL for Card Service SOAP" + "\n" + value);
               ctx.respond(asBlocks(
       ...
       app.blockAction("Jenkins Job SOAP", (req, ctx) -> {
           String value = req.getPayload().getActions().get(0).getValue();
           if (req.getPayload().getResponseUrl() != null) {
               ctx.respond("Please find the below Jenkins Job for Card Service SOAP"+ "\n" + value);
               ctx.respond(asBlocks(
                       section(section -> section.text(markdownText("Do you want to know anything else about this service" + "\n"))),
                       actions(actions -> actions
                               .elements(asElements(
